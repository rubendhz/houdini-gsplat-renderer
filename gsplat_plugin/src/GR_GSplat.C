...
bool GR_PrimGsplat::initAllSHHandles(const GU_Detail *gdp, SHHandles& handles) {
	...
	if (handles.fallback)
	{
		GSplatLogger::getInstance().log(GSplatLogger::LogLevel::WARNINGX, "%s", "Spherical harmonics attributes 'sh1, sh2, ..., sh15' not found. Trying fallback f_rest_X attributes...");
		...
		if (!handles.valid)
		{
			GSplatLogger::getInstance().log(GSplatLogger::LogLevel::WARNINGX, "%s", "Spherical harmonics fallback 'f_rest_X' attributes not found.");
		}
		else
		{
			GSplatLogger::getInstance().log(GSplatLogger::LogLevel::INFOX, "%s", "Spherical harmonics fallback 'f_rest_X' found.");
		}
	}
	...
	const GR_UpdateParms      &p)
{
	...
	if (!cdAttr) 
	{
		GSplatLogger::getInstance().log(GSplatLogger::LogLevel::ERRORX, "%s", "Color attribute 'Cd' not found!");
	}
	...
	{
		GSplatLogger::getInstance().log(GSplatLogger::LogLevel::ERRORX, "%s", "Opacity attribute not found! (tried 'opacity' and 'Alpha')");
	} 
	...
	if (!scaleAttr) 
	{
		GSplatLogger::getInstance().log(GSplatLogger::LogLevel::ERRORX, "%s", "Scale attribute 'scale' not found!");
	}
	...
	if (!orientAttr) 
	{
		GSplatLogger::getInstance().log(GSplatLogger::LogLevel::ERRORX, "%s", "Orientation attribute 'orient' not found!");
	}
	...
	if (shOrderHandle.isValid())
	{
		...
		{
			GSplatLogger::getInstance().log(GSplatLogger::LogLevel::ERRORX, "Spherical harmonics order requested: %d. Allowed values are 0, 1, 2, 3. Contribution will be disabled.", myShOrder);
			myShOrder = 0;
		}
	}
}
